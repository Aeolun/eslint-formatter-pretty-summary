...
const fakeMessages = (desiredSeverity, desiredCount) => {
	const ofDesiredSeverity = messages.filter(
		({ severity }) => severity === desiredSeverity
	);
...
test("output", (t) => {
	...
	t.regex(
		stripAnsi(output),
		/error {3}1:1 {2}AVA should be imported as test. {6}ava\/use-test/
	);
});
...
// 	t.regex(
// 		stripAnsi(output),
// 		/error {2}AVA should be imported as test. {6}ava\/use-test/
// 	);
// });
...
// 	const indexes = [
// 		sanitized.indexOf("warning   1:1"),
// 		sanitized.indexOf("warning  10:2"),
// 		sanitized.indexOf("error   3:1"),
// 		sanitized.indexOf("error  30:1"),
// 		sanitized.indexOf("error  40:5"),
// 		sanitized.indexOf("error  40:8"),
// 	];
// 	console.log(output);
// 	t.deepEqual(
// 		indexes,
